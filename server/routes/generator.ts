...
try {
  // 使用动态导入
  const aiClientModule = await import("./aiClient.ts");
  // 显式检查 analyzeStream 是否存在且为函数，避免使用不存在的全局 assert
  if (!aiClientModule || typeof aiClientModule.analyzeStream !== "function") {
    throw new Error("aiClient.analyzeStream 未实现");
  }
