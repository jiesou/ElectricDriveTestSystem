...
**日志格式化：**
后端将原始日志数据格式化为易于理解的 Markdown 文本。以下是日志格式模板（方括号内为实际数据占位符）：
```
## 学生: [客户机名称] ([IP地址])
...
### 4.4 未来扩展方向：MCP（Model Context Protocol）深度集成

为了让 AI 更深入理解测验过程，系统架构已预留 MCP 扩展接口。以下功能目前处于规划阶段：

**MCP 的扩展潜力：**
MCP（Model Context Protocol）是一种让 AI 模型访问外部工具和数据的协议。计划通过 MCP 实现：
